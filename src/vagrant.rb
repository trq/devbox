class DevBox
    def DevBox.configure(config, settings)

        config.vm.box      = settings["box"]      ||= "devbox"
        config.vm.box_url  = settings["box_url"]  ||= "http://cloud-images.ubuntu.com/vagrant/trusty/current/trusty-server-cloudimg-amd64-vagrant-disk1.box"
        config.vm.hostname = settings["hostname"] ||= "devbox.lcl"
